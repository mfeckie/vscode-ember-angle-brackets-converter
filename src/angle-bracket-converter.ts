...
function shouldSkipAttribute(key, config) {
  if (
    ...
    config.skipAttributesThatMatchRegex.length
  ) {
 ...
 */
function transformTagName(tagName: string) {
...
function shouldSkipFile(fileInfo, config) {
  ...
  if (
    config.skipFilesThatMatchRegex &&
    config.skipFilesThatMatchRegex.test(source)
  ) {
    logger.warn(
...
function transformLinkToAttrs(params) {
  ...
  if (isQueryParam(firstParamInput)) {
    firstParamOutput = b.attr(
      "@query",
      b.mustache(b.path("hash"), [], firstParamInput.hash)
    );
    ...
    firstParamOutput = b.attr(
      "@route",
      b.mustache(
        firstParamInput.path,
        firstParamInput.params,
        firstParamInput.hash
      )
    );
  } else {
  ...
  if (params.length === 1) {
    ...
    if (secondParamInput.type === "SubExpression") {
      ...
      let _modelParam = b.attr(
        "@model",
        transformModelParams(secondParamInput)
      );
...
function shouldIgnoreMustacheStatement(
  fullName: string,
  config?: any,
  invokableData?: any
) {
  return KNOWN_HELPERS.includes(fullName) || config.helpers.includes(fullName);
}
...
function nodeHasPositionalParameters(node) {
  if (node.params.length > 0) {
    ...
    if (
      ["StringLiteral", "NumberLiteral", "SubExpression"].includes(
        firstParamType
      )
    ) {
      return true;
...
function subExpressionToMustacheStatement(subExpression) {
  return b.mustache(
    subExpression.path,
    subExpression.params,
    subExpression.hash
  );
}

function transform(
  fileInfo: { source: string },
  config?: any,
  invokableData?: any
) {
  config = config || {};
...
function transformToAngleBracket(
  fileInfo: { source: string },
  config?: any,
  invokableData?: any
) {
